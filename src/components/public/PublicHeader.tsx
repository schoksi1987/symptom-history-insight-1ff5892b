...
export function Wordmark({ className = "" }: { className?: string }) {
  return (
    <span className={`flex items-baseline gap-1.5 ${className}`}>
      <span className="text-xl font-bold text-foreground">Predict Disease</span>
      <span className="text-sm font-medium text-muted-foreground">by symptom.ai</span>
    </span>
  );
}
  ...
  return (
    <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        <Link to="/" className="flex items-center gap-2" aria-label="Predict Disease by symptom.ai — home">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          <Button variant="outline" onClick={() => navigate(approved ? "/dashboard" : "/auth")}>
            {approved ? "Open Workspace" : "Sign In"}
          </Button>
          <Button onClick={tryDemo}>Try the Clinical Demo</Button>
        </div>
