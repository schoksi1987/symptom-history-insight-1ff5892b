...
- New table `account_status`: `user_id` (UNIQUE, references `auth.users(id)` ON DELETE CASCADE), `status` (NOT NULL, constrained to pending/approved/rejected, default pending), `is_demo` and `demo_requested` (NOT NULL, default false), `requested_role`, `organization`, `purpose`, `approved_by`, `approved_at`, `rejected_by`, `rejected_at`, `created_at` and `updated_at` (NOT NULL, default now(), with the existing updated-at trigger). Grants for `authenticated` and `service_role`; RLS lets a user read only their own row, blocks users from writing status/is_demo/approved_* /rejected_* (enforced by a trigger that rejects privileged-column changes from non-admins), and lets `has_role(auth.uid(), 'admin')` read and manage all rows.
